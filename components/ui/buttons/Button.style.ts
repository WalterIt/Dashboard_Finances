...
const normalButton = styled.button<ButtonType>`
  ...
  position: relative;
  background-color: ${({ variant, color, theme }) => (variant === 'text' || variant === 'outlined') ? 'transparent' : color ? skinColor(color) : theme.primaryColor.color};
  box-sizing: border-box;
  // border
  border: 1.8px solid ${({ variant, color, theme }) => variant === 'outlined' ? (color ? skinColor(color) : theme.primaryColor.color) : 'transparent'};
  text-align: center;
  justify-content: center;
  vertical-align: middle;
  // color
  color: ${({ theme, color, variant }) => variant === 'outlined' || variant === 'text' ? color ? skinColor(color) : theme.primaryColor.color : "#fff"};
  font-size: 0.875rem;
  font-weight: 500;
  ...
  &:hover {
    background: ${({ theme, color, variant }) => variant === 'contained' ? genColorShades(color ? color : theme.primaryColor.color, { total: 1, intensity: 9 }).toString() : genColorShades(color ? color : theme.primaryColor.color, { total: 1, intensity: 2 }).toString()};
...
export const iconButton = styled(normalButton) <IconButtonProps>`
  ...
  color: ${({ variant, color }) => color ? color : variant === "contained" ? "#fff" : 'inherit'};
  padding: 0px;
  // border: ${({ variant }) => variant === 'outlined' ? 1.8 : 0}px;
  user-select: none;
  overflow: hidden;
  z-index: 1;
