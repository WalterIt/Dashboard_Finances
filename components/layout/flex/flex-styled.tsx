import React, { FC } from "react";

interface LayoutFlexProps {
  direction?: "row" | "column" | "column-reverse" | "row-reverse";
  wrap?: boolean | "evenly" |  "wrap-reverse";
  gap?: "sm" | "regular" | "medium" | "large";
  alignment?: "center" | "bottom" | "top" | "evenly" | "start" | undefined;
  justify?: "center" | "around" | "between" | "start" | "end" | "evenly" | undefined;
  children: React.ReactNode;
}

const LayoutFlex: FC<LayoutFlexProps> = ({
  direction = "row",
  wrap = false,
  gap = "regular",
  alignment = "center",
  justify = "start",
  children,
}) => {
  const flexWrap = wrap === "evenly" ? "wrap-reverse" : wrap ? "wrap" : "nowrap";

  return (
    <div
      className={`flex flex-${direction} ${flexWrap} gap-${gap} items-${alignment} justify-${justify}`}
    >
      {children}
    </div>
  );
};

export default LayoutFlex;










// import styled from 'styled-components';
// import FlexType from './flex-type';
// const layoutFlex = styled.div<FlexType>`
//     min-width: 100%;
//     width: 100%;
//     display: flex;
//     // direction
//     flex-direction: ${props => props.direction};
//     // wrap
//     flex-wrap: ${props => props.wrap};
    
//     // gap
//     gap: ${({gap})=> gap === 'sm' ? '8px' : gap === 'regular' ? '16px' : gap === 'medium' ? '24px' : gap === 'large' ? '36px' : ''};
//     // items alignment
//     align-items: ${props => props.alignment === 'center' ? 'center' : props.alignment === 'bottom' ? 'flex-end' : 'flex-start'};
//     // content alignment
//     justify-content: ${props => !props.justify ? 'flex-start' :  props.justify === 'center' ? 'center' : props.justify === 'around' ? 'space-around' : props.justify === 'between' ? 'space-between' : 'space-evenly'};
// `;
// export default layoutFlex;